...
External evidence plus three local sanitized derivative fixtures, two local sanitized-derived negative fixtures, and one retained superseded raw-derived audit packet.
...
| --- | --- | --- | --- |
| local workspace inspection | local | three extracted sanitized derivative fixtures exist under `fixtures/scrivener/ambiguous/` and `fixtures/scrivener/positive/`, two extracted sanitized-derived negative packets exist under `fixtures/scrivener/negative/`, and one earlier raw-derived negative packet is retained only as superseded provenance history | high |
...
8. The `.scrivx` file appears necessary control material, but it is not proven locally sufficient project truth.
...
- broader irregular coverage beyond one sanitized-derived corrupted-authority case and one sanitized-derived missing-content case
